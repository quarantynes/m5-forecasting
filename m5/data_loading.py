...
def compose_data():
    ...
    assert units_sales.shape == (30490, 1913,)
    ...
    assert units_sales_feature.shape == (30490, 1913 - window,)
    assert prices.shape == (30490, 1913 - window,), prices.shape
...
def split_data(split_day=-28, sequence_training=True):
    ...
    if sequence_training:
        # for sequence training, we need the whole dataset to evaluate evaluation points
